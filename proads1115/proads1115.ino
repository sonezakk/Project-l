...
void loop()
{
  ...
  if ((millis() - Showblynk) > 1000)
  {
    Showblynk = millis();
    Blynk.run();
    timer.run();
  }
  static unsigned long ShowPH = millis();
  if ((millis() - ShowPH) > 30000)
  {
    ShowPH = millis();
    sensorPh();
    // process(sensorDo(), 5.50, 5.00);
  }
  static unsigned long Showtime = millis();


  if ((millis() - Showtime) > 13000)
  { //1*60*1000
    Showtime = millis();
    sensorDo();
...
//process
void process(float sensordata, float max, float min)
{
  // serialprint("input sensordata > " + String(sensordata) + ", max > " + String(max) + ", min > " + String(min));

  //if (max <= sensordata)
  if (sensordata >= max)
  {
    //serialprint("Max stop sensordata <= max :: sensordata  " + String(sensordata) + " max ::" + String(max));
    controlmoter(0);
    led.off();
    LINE.notify("ปั้มหยุดทำงาน");
    LINE.notify("PH : " + String(sensorPh()) + " Do : " + String(sensorDo()) + " mg/L" + "temp : " + String(sensorTemp()) + "'C");
  }

  if (sensordata <= min)
  {
    //serialprint("Min start sensordata <= min :: sensordata  " + String(sensordata) + " min ::" + String(min));
